...
```text
input:เป็นข้อความ str
output: เป็นสระที่มีในข้อความinput ซิ่งมีทั้งหมด5ตัวคือ aeiou แยกกันคนละตัวแปร
constraints: ข้อความความยาว อยู่ในช่วง 1<=n<1000 และแสดงผลลัพธ์เป็นจํานวนสระที่นับได้
```
...
```text
1รับค่าเป็นstr เข้ามาแล้วทำlower ให้เพื่อให้ง่านต่อการเทียบ
2.สร้างลิสต์2ไว้2อันเพื่อเก็บ 1สระ 2จำนวนสระที่นับได้
3.ทำการใช้for loop ในtext หรือinputที่รับมา เพื่อเช็คทีละตัว โดยเงื่อนไขคือถ้ามีสระตัวใดตรงกับลิสต์สระที่สร้างไว้ ให้ทำการ+1ใส่ตำแหน่งเดียวกันกับที่สระอยู่ จะได้ตำแหน่งได้จากการใช้index เพื่อหาตำแหน่งว่าจะบวกใส่ตำแหน่งไหนในลิสต์ดี
4.เมื่อloopอ่านค่าtextหมดแล้ว ทำการใช้for loop อีกครั้งตามจำนวนสมาชิกในลิสต์ของสระที่เก็บ เพื่อให้สามารถปริ้นออกมาตามลำดับสระได้ตรง
5.โดยใส่เงื่อนไขว่า ถ้า ตำแหน่งอ่านในลิสต์มีค่ามากกว่า0 ให้ทำการprint ชื่อสระ ตามด้วย จำนวนที่อ่านได้ นอกเหนือจากนั้นไม่ต้องทำอะไร
```
...
```text
ทดสอบตัวพิมพ์ใหญ่ทั้งหมด เพื่อเช็คว่าสระจะอ่านได้มั้ย
```
...
```text
ลองใส่ตัวเลข อักษรพิเศษ ว่าจะทำงานได้มั้ย
```

Input:

```text
12KM34ITL56_PURICH_69070160
```
...
```text
ใส่ตัวเลขทั้งหมดเพื่อดูว่าจะทำงานได้มั้ย
```
