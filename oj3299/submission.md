...
```text
input: รับจำนวนเต็มสองตัวคือ L และ N ความหนาของแถบและจำนานช่องที่จะปลูก

output: แถบที่ปลูกต้นไไม้ในช่อง N โดยนับแถบแรกเป็น 1

Constraints: แปลงไม่จำกัดขนาด เลยไม่ต้องกลัวว่าจะปลูกล้านขอบ
ระวังว่า N = 1 ได้ต้อง1 และกรณรีทีN ตกตรงช่องสุดท้ายของแถบพอดี ต้องนับเป็นแถบนั้น
```
...
```text
1 รับค่า LกับN เข้ามาโดยใช้ split ตัดช่องว่าง แล้วทำให้เป็นจำนวนเต็ม
2 ตั้งตัวแปรนับ count = 0 ไว้นับว่าปลูกไปแล้วกี่ช่อง และ band = 1 ว่าอยู่แปงที่เท่าไหร่
3ดูว่าแถบที่ 1 มันวนทีละคอลัมน์รึป่าว แต่ละคอลัมน์ปลูกแนวตั้งไม่เกิน L ช่อง ทุกครั้งที่ปลูก +1 count
4 ปลูกจนชนมุม คือแถวไปถึงแนวบนสุด ถือว่าจบแถบนั้นแล้วให้ +1 band
5 ทำซ้ำไปเรื่อยๆ จนกว่าจะเจอช่อง N
```
...
```text
N ตกตรงช่องสุดท้ายของแถบพอดี ถ้าใช้ < กับ <= สลับกัน ผมเลยเลือกcaseนี้
```
...
```text
N ตกตรงช่องสุดท้ายของแถบพอดี ถ้าใช้ < กับ <= สลับกัน เพื่อเช็คว่าโปรแกรมจะปัดตกไปแปลงต่อไปมั้ย
```
...
```text
N ตกตรงช่องสุดท้ายของแถบพอดี ถ้าใช้ < กับ <= สลับกัน เพื่อเช็คว่าโปรแกรมจะปัดตกไปแปลงต่อไปมั้ย
ช่องถัดไปหนึ่งช่องต้องขึ้นแถบใหม่
```
