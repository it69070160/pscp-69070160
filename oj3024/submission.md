...
```text
input :  เป็นค่า float มา 2 ค่า คือ 1คะแนนรวมของทั้งสามคน และ 2คะแนนสูงสุดของทั้งสามคนนี้
output: แสดงผลลัพธ์ว่ามีโอกาสเกิด Surprising จากการที่คะแนนของคนที่มากสุดห่างจากคนที่น้้อยสุดอยู่น้อยกว่า 2 คะแนน โดยพิมพ์ Surprising ถ้ามีโอกาสเกิด หรือ Not surprising ถ้าไม่มีโอกาสเกิด
constraints: คะแนนของทุกคนต้องอยู่ในช่วง 1-10 
```
...
```text
1.อ่านค่าที่รับมาเป็นfloatก่อน 2 ค่า
2.ทำการหาminscoree โดยการเอา score(คะแนนทั้งหมด3คน) - (2*maxscore) คือหาคะแนนที่น้อยที่สุดที่เป็นไปได้
3.สร้างเงื่อนไขที่minscore จะน้อยกว่า0 ให้จับมาเท่ากับ0ก่อนเข้าเงื่อนไขถัดไป
4.สร้างเงื่อนไข เช็คคะแนนว่าห่างมากกว่า 2มั้ย ถ้าห่างแสดง Surprising
5.เงื่อนไข lese เช็คว่าห่างกันน้อยกว่า 2 แสดงผล Not surprising
6.ทำการทดสอบtestcase ตามที่โจทย์ให้มา ว่าเหมือนกันมั้ย
```
...
```text
ใช้วิธีแรกที่คิดได้ แล้วค่อยๆเขียน อาจจะติดตรงminscore < 0 ไปบ้าง แต่ก็สามารถแก้caseนี้ไปได้
```
...
```text
ทดสอบถ้าคะแนนคนที่มากสุดน้อย จะแสดงผลว่า Surprising มั้ย
```
...
```text
ทดสอบว่าห่างกันเกินสองคะแนน ไม่ใช่มากกว่าเท่ากับ 2  คะแนนใช่มั้ย
```
...
```text
ทดสอบเคสทั่วไปที่เกิดขึ้นได้จริง
```
