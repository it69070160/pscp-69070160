...
```text
input: จำนวนนับ4ค่า เป็นจำนวนจริง 
output: เป็นจำนวนที่สามารถหารด้วย d แล้วเหลือเศษ r ในช่วงจำนวน [A,B]
constraints: ระวังเรื่องTime Limit เพราะถ้าค่ามากการใช้ forloopอาจจะTime Limit
```
...
```text
1.รับค่ามา4ค่าเป็นจำนวนเต็ม
2.สร้างลิสต์จำนวนนับระหว่าง a ถึง b+1
3.ทำการfor loop ในลิสต์ที่สร้างขึ้น แล้วทำการ+1ตัวแปรใหม่ หากหารdแล้วเหลือเศษ r จนจบloop
4.แสดงผลลัพธ์
```
...
```text
เปลี่ยนมาใช้วิธีใหม่เพราะวิธีแรกที่ใช้มันติด Time Limit 
1.รับค่ามา4ค่าเป็นจำนวนเต็ม
2.ทำการหาว่ามีกี่ตัวตั้งแต่1จนถึงb โดยการ นำ bไปลบr เพื่อให้หารลงตัวพอดีกับการหาร d (count_b) แล้ว+1ตามสูตรคณิตศาสตร์ เพื่อชดเชยจำนวนเริ่มต้น
3.หาอีกค่าที่เริ่มตั้งแต่1 แต่จบที่a เพราะ เป็นค่าที่ไม่ต้องการ (count_a) 
4.นำค่าcount_b ไปลบกับ count_a เพื่อจะได้รู้ว่ามีกี่ตัวในช่วง a ถึง b
```
...
```text
ทดสอบช่วงตัวเลขเยอะๆ เพื่อเช็คว่าTime Limitไมั้ย
```
...
```text
ตัวแรกเริ่มต้นกับสุดท้ายเป็นตัวเดียวกัน เพื่อเช็คว่าโปรแกรมยังทำงานได้มั้ย
```
...
```text
ทดสอบช่วงจำนวนนับที่น้อยกว่าตัวหาร 
```
